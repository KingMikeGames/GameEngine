#include "Sprite.h"
#include "Vertex.h"

#include <cstddef>
#include "ResourceManager.h"
#include <iostream>

Sprite::Sprite()
{
    //Always initialize your buffer IDs to 0
    _vboID = 0;
}


Sprite::~Sprite()
{
    //Always remember to delete your buffers when
    //you are done!
    if (_vboID != 0) {
        glDeleteBuffers(1, &_vboID);
    }
}

//Initializes the sprite VBO. x, y, width, and height are
//in the normalized device coordinate space. so, [-1, 1]
void Sprite::init(Rectangle rect, std::string texturePath) {
    //Set up our private vars
	_rect.position = rect.position;
    _rect.height = rect.height;
	_rect.width = rect.width;

	_texture = ResourceManager::getTexture(texturePath);



    //Generate the buffer if it hasn't already been generated
    if (_vboID == 0) {
        glGenBuffers(1, &_vboID);
    }

    //This array will hold our vertex data.
    //We need 6 vertices, and each vertex has 2
    //floats for X and Y
    Vertex vertexData[6];

    //First Triangle
	vertexData[0].position = glm::vec3(_rect.position.x + _rect.width, _rect.position.y + _rect.height, _rect.position.z);
	vertexData[0].uv = glm::vec2(1.0f, 1.0f);

	vertexData[1].position = glm::vec3(_rect.position.x, _rect.position.y + _rect.height, _rect.position.z);
	vertexData[1].uv = glm::vec2(0.0f, 1.0f);

    vertexData[2].position = glm::vec3(_rect.position.x, _rect.position.y, _rect.position.z);
	vertexData[2].uv = glm::vec2(0.0f, 0.0f);
    //Second Triangle
	vertexData[3].position = glm::vec3(_rect.position.x, _rect.position.y, _rect.position.z);
	vertexData[3].uv = glm::vec2(0.0f, 0.0f);

    vertexData[4].position = glm::vec3(_rect.position.x + _rect.width, _rect.position.y, _rect.position.z);
	vertexData[4].uv = glm::vec2(1.0f, 0.0f);

    vertexData[5].position = glm::vec3(_rect.position.x + _rect.width, _rect.position.y + _rect.height, _rect.position.z);
	vertexData[5].uv = glm::vec2(1.0f, 1.0f);

    //Set all vertex colors to magenta
    for (unsigned i = 0; i < 6; i++) {
		vertexData[i].color = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
    }

	for (unsigned i = 0; i < 6; i++) {
		vertexData[i].normal = glm::vec3(0.0f, 0.0f, 0.0f);
	}

    //Tell opengl to bind our vertex buffer object
    glBindBuffer(GL_ARRAY_BUFFER, _vboID);
    //Upload the data to the GPU
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertexData), vertexData, GL_STATIC_DRAW);

    //Unbind the buffer (optional)
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}

//Draws the sprite to the screen
void Sprite::draw() {

	glBindTexture(GL_TEXTURE_2D, _texture.id);

    //bind the buffer object
    glBindBuffer(GL_ARRAY_BUFFER, _vboID);

    //Tell opengl that we want to use the first
    //attribute array. We only need one array right
    //now since we are only using position.
    glEnableVertexAttribArray(0);

    //This is the position attribute pointer
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, position));
    //This is the color attribute pointer
    glVertexAttribPointer(1, 4, GL_FLOAT, GL_TRUE, sizeof(Vertex), (void*)offsetof(Vertex, color));
	//normals
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_TRUE, sizeof(Vertex), (void*)offsetof(Vertex, normal));
	//uv
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, uv));

    //Draw the 6 vertices to the screen
    glDrawArrays(GL_TRIANGLES, 0, 6);

    //Disable the vertex attrib array. This is not optional.
    glDisableVertexAttribArray(0);

    //Unbind the VBO
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}